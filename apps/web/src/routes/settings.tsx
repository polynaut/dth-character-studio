...
import { ArrowLeft, FolderSearch, Save } from 'lucide-react'
    ...
    <main className="mx-auto max-w-4xl p-8">
      ...
      <header className="mb-8">
        ...
        <p className="mt-1 text-muted-foreground">
          Machine-specific folders — stored locally in <code>data/settings.json</code>, never
          committed.
        </p>
      </header>
      ...
      <section className="mb-8 space-y-5 rounded-lg border bg-card p-5">
        <div>
          <Label className="mb-1">Current DTH release folder (or Poses folder)</Label>
          <Input
            value={settings.dthPosesFolder}
            placeholder="X:\_3d\_resources\_DazToHue\Releases\Release 2.4.3"
            onChange={(e) => setSettings((s) => ({ ...s, dthPosesFolder: e.target.value }))}
          />
        ...
        <div>
          <Label className="mb-1">DazToHue-Scripts folder</Label>
          <Input
            value={settings.dazScriptsFolder}
            placeholder="D:\Development\DazToHue-Scripts"
            onChange={(e) => setSettings((s) => ({ ...s, dazScriptsFolder: e.target.value }))}
          />
          <p className="mt-1 text-xs text-muted-foreground">
            Generated Daz workflow files are written here, next to DthWorkflow.dsa, so they are
            directly runnable from Daz Studio.
          </p>
        </div>
