...
export const poseColumns: Array<ColumnDef<RomPose, any>> = [
  ...
  columnHelper.accessor('boneScaleRef', {
    ...
    cell: ({ getValue, row, table }) => {
      const meta = table.options.meta as PoseTableMeta
      // Match the row: an overridden row (edited base frame or the override's own
      // appended frame) reads green, so its active checkbox is green too — not the
