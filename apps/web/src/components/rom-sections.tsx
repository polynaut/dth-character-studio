      ...
      {ROM_SECTIONS.map((section) => {
        ...
        return (
          ...
          <div key={section} className={`rounded-lg border ${effectiveEnabled ? '' : 'opacity-60'}`}>
            {/* Sticky section title: pins right below the page's sticky "chrome"
                (header + tabs) via the fixed `--chrome-h` — a constant, since the
                header is always collapsed by the time a tier pins this far down. z
                below the chrome's z-10. Solid bg so rows can't show through; rounded-t
                so the bg doesn't square out the card's top corners at rest. NB: the
