...
def launch():
    ...
    idiom, so the window opens first and the exports run in a visible session;
    the studio's result-file watch covers progress either way ("Houdini
    opening…" simply lasts until the deferred batch writes its first state).
    Without the module (hython / no UI event loop) there is no window to wait
