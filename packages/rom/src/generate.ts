...
/*
 * NOTE — a standalone groom (.abc) export script was measured OUT (July 17 2026,
 * probe series on Karen + dForce Black Tie Hair): Daz's own Alembic add-on is a
 * dead end (DS6's export manager has no `findExporterByExtension`, and the
 * pipeline's .abc comes from the DTH Exporter anyway), and the DTH Exporter,
 * while it ACCEPTS an unparented hair cap as its root (fit survives
