...
// DTH Workflow for ${commentSafe(character.name)} — generated by DTH Character Studio${character.studioVersion ? ` v${commentSafe(character.studioVersion)}` : ''}
  ...
  for (const { section, config } of customSections(character.sections)) {
    if (section === 'FBM' || section === 'MISC') {
      ...
      for (const group of config.groups) {
        for (const pose of group.poses) {
          frame += 1
          rows.push(
            `${keyword},${frame},${sanitizePoseName(pose.name)},${csvSafe(pose.referenceFbx ?? '')}`,
          )
        }
      }
      continue
    ...
    for (const group of config.groups) {
      ...
      for (const pose of group.poses) {
        ...
        if (section === 'GEN') rows.push(`GEN,${frame},${name},${csvSafe(pose.referenceFbx ?? '')}`)
...
export function referenceFrames(character: Character, frames: PresetFrames): Array<number> {
  // presetEndFrame is -1 when there is NO preset block; the pre-increment below
  // then correctly lands the first custom pose at frame 0 (matching the Daz
  // runtime's `startFrame = 0` for a base-less ROM). Clamping -1 → 0 here shifted
  // every custom frame by one and desynced the CSV from Daz — do NOT re-add it.
  let frame = presetEndFrame(character, frames)
...
export function toPoseAssetCsv(
  ...
  return {
    fileName: poseAssetFileName(character),
    // lastPresetFrame is -1 with no base ROM → first custom pose lands at frame 0
    // (see referenceFrames). NEVER clamp to 0 — that desyncs the CSV from Daz.
