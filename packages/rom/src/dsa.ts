...
${refFramesBlock}${
    ...
    var dthMotion = dthExportLanded ? dthMotionSummaryVerdict(dthExportDir + "/" + dthExportName + ".log") : null;
    var dthMotionStatue = dthMotion !== null && dthMotion.zero == dthMotion.nodes;
    if (dthMotionStatue) dthExportLanded = false;
    ...
    if (dthMotionStatue) {
        ...
        if (dthMotion !== null && dthMotion.best < 0.9) {
            ...
            dthExportLogProblem("Partial evaluation staleness: even the liveliest node's mesh changed on only " + dthMotionPct + "% of the ROM's frames (exporter staleness - see the motion summary in " + dthExportName + ".log). Treat this export set as suspect and re-run the export.", true);
        }${
