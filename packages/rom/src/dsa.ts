...
/** The hidden per-character HAIR-ONLY script (DTH Export's "Hair items only"
 *  mode): the standalone Export_Hair pass as an unattended Runner carrier —
 *  each job row opens its source scene and exports EACH hair item on its own
 *  (one `<Name>_Hair_<item>_grooms.abc` per item), nothing else: no ROM, no
 *  skeleton/mesh export, no CSV. Dot-prefixed so Daz's Content Library never
 *  shows it. */
export const BULK_HAIR_EXPORT_SCRIPT = '.Bulk_Hair_Export.dsa'
...
/**
 * The motion-summary parser, as CARRIER text (runtime v102).
 *
 * It lives here rather than in DthUtils.dsa for the same reason
 * `dthExportLogProblem` does: the `Export_` carriers and `.Bulk_Export_Only`
 * include NO runtime at all, and they are the primary export path. A runtime
 * function called from one of them is a ReferenceError that aborts the script
 * right after a successful export — before the backups are finished — which
 * turns the next run's sweep into a deletion of the good set (the v99 corpse
 * rule). Anything the export block calls must be DEFINED in the export block.
 *
 * Exported so the sandbox test runs THIS text instead of a copy of it.
 */
export const MOTION_SUMMARY_HELPER = `// The DTH Exporter's own "Alembic ROM motion summary" (exporter >= 2.1.9):
...
var dthExportLogProblem = function (dthLogMsg, dthLogWarn) {
    ...
    try {
        ...
        if (!dthLogRec || typeof dthLogRec != "object") {
            ...
            dthLogRec = { logVersion: 1, character: ${dazJson(character.name)}, ok: dthLogWarn ? true : false, errors: [], warnings: [], failedMorphs: [] };
        }
...
`
  return `${alertHelper}${MOTION_SUMMARY_HELPER}var dthExportAction = MainWindow.getActionMgr().findAction("DazToHueExporterAction");
if (!dthExportAction) {
...
export function toGroomExportScriptDsa(
   ...
   *  {@link runtimeDirSnippet}. '' = no fallback (pure/web contexts). */
  runtimeRootAbs = '',
  /** Set by the bulk twin ({@link toBulkHairExportScriptDsa}) and ONLY then:
   *  the Runner executes that one unattended, so every modal becomes a print
   *  (a dialog would block the whole batch on a click nobody is present to
   *  make), the interrupt flag is probed, and the finished pass reports to the
   *  verbose progress log. Absent = the visible standalone script, unchanged. */
  carrier?: { metaDirAbs?: string; progressLogPath?: string },
): GeneratedFile {
  const exportDir = character.exportPath.trim().replace(/\\/g, '/')
  const groomMap = groomSceneMap(character)
  const unattended = carrier !== undefined
  ...
  // The carrier plumbing (the export-only carrier's shape): the baked
  // progress-log/cancel paths + their inline helpers, and the interrupt check
  // as its own guard branch. All empty on the standalone script — its emitted
  // bytes must not change under a runtime whose only news is the bulk twin.
  const carrierVars = unattended
    ? `var dthProgressLogPath = ${dazJson(carrier.progressLogPath ?? '')};
// The studio's interrupt flag for this character — see EXPORT_CANCEL_FILE.
var dthCancelPath = ${dazJson(cancelFlagPath(carrier.metaDirAbs))};
`
    : ''
  const carrierFns = unattended ? `${dthProgressSnippet()}\n${dthCancelSnippet()}\n` : ''
  const cancelBranch = unattended
    ? ` else if (dthCancelRequested()) {
    // Interrupted before this row's turn — skip the scene and say so (the
    ...
    : ''
  // Unattended guard failures print + close the row's progress; attended ones
  // stay the modals the manual script always showed.
  const fail = (message: string, progress: string): string =>
    unattended
      ? `print("DTH Character Studio: ${message}");
    dthProgressLog(100, ${dazJson(progress)});`
      : `MessageBox.critical("${message}", "DTH Character Studio", "&OK");`
  const content = `// DAZ Studio version 4.22.0.16 filetype DAZ Script
...
// via the DTH Exporter, restores the scene. ${unattended ? `The Runner executes this one\n// unattended (DTH Export's "Hair items only" mode); the ROM is NOT needed.` : `Run it on the character's scene with\n// the figure selected; the ROM is NOT needed.`}
${runtimeIncludeBlock(runtimeRootAbs, indexSync)}
${carrierVars}${sceneGuardSnippet(character)}
${openSceneFileSnippet()}${romAnimationSourceSnippet(romAnimationSourceMap(character))}
${carrierFns}var dthAction = MainWindow.getActionMgr().findAction("DazToHueExporterAction");
${figureAutoSelectSnippet(character.genesis)}var dthSceneLinkErr = dthSceneLinkError();
if (dthSceneLinkErr) {
    ${
      unattended
...
}${cancelBranch} else if (!dthAction) {
    ${fail('DazToHue Exporter Action not found - install the DTH Exporter Plugin v2.0+.', 'exporter plugin missing - nothing was exported')}
} else if (!dthFig || !dthFig.inherits("DzNode")) {
    ${fail(`No ${character.genesis} figure found in the scene - load the character's scene and re-run.`, 'no figure found - nothing was exported')}
} else {
...
/**
 * The hidden HAIR-ONLY carrier ({@link BULK_HAIR_EXPORT_SCRIPT}) the Runner
 * executes for DTH Export's "Hair items only" mode: the SAME per-item hair
 * pass as the standalone Export_Hair script ({@link toGroomExportScriptDsa}'s
 * builder, so the two can never drift), run unattended — prints instead of
 * modals, the interrupt flag probed, the finished pass reported to the verbose
 * progress log (the 2-step job-row scale: the Runner's scene-open line, then
 * this script's 100). No ROM, no skeleton/mesh export, no CSV — one
 * `<Name>_Hair_<item>_grooms.abc` per hair item of each opened scene. Emitted
 * with an export dir like its bulk siblings (the pass needs somewhere to
 * export to); which scenes actually run is the panel's selection — it lists
 * only the ones whose "Export hair items" switch is on.
 */
export function toBulkHairExportScriptDsa(
  character: Character,
  scenesRootAbs?: string,
  /** Scan-sync settings — see {@link IndexSyncOptions}. */
  indexSync?: IndexSyncOptions,
  /** The character's `.dcsmeta` folder (host-resolved) — where the interrupt
   *  flag lives ({@link cancelFlagPath}). Undefined in pure/web contexts,
   *  which makes the carrier uninterruptible, like every other carrier. */
  metaDirAbs?: string,
  /** See {@link buildRomScriptDsa}. */
  progressLogPath = '',
  /** See {@link toGroomExportScriptDsa}. */
  runtimeRootAbs = '',
): GeneratedFile {
  const built = toGroomExportScriptDsa(character, scenesRootAbs, indexSync, runtimeRootAbs, {
    metaDirAbs,
    progressLogPath,
  })
  // Hidden (dot-prefixed) → no Content Library tile, no icon artwork.
  return { fileName: BULK_HAIR_EXPORT_SCRIPT, content: built.content, target: 'daz' }
}
  ...
  return [
    ...
    ...(exportSet
      ? [
          ...
          // …and the hair-only carrier (DTH Export's "Hair items only" mode):
          // the per-item hair pass alone, unattended. With the export dir like
          // its siblings — NOT gated on the hair lists, so a scene whose list
          // is filled after the last save still has its carrier on disk (the
          // pass itself no-ops per scene without one).
          toBulkHairExportScriptDsa(
            character,
            scenesRootAbs,
            indexSync,
            metaDirAbs,
