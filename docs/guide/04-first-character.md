...
- **Daz scenes** — the primary plus any number of outfit/look variants: **drop a
  ...
  unlinked; extras can. Each scene's open icon is a menu: **Open scene**; then
  **Open last ROM** once a
  [ROM animation](./05-rom-in-daz.md#what-a-run-exports) is saved — there
  whenever the file is on disk, whether or not it was built from the character as
  it reads now; and **Generate new ROM**, always offered.
