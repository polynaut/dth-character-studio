...
3. For each row: **open the scene (or a new empty one when the row has no
   scene path) → run the script with the argument `bulk-export` → discard the
   scene changes → next row.**
...
- The **scene column may be empty** (`,X:\…\Some_Script.dsa`): such a row's
  script runs in a **new empty scene** — start one (File → New equivalent,
  discarding whatever is open, never saving) and execute the script in it.
  The script column is never empty; a row with an empty script path is invalid
  and skipped (logged).
