...
3. For each row: **open the row's scene — or explicitly a NEW EMPTY scene when
   the row has no scene path → run the script with the argument `bulk-export`
   → discard the scene changes → next row.**
...
open scene's overrides itself, and `bulk-export` makes it export). When Daz
