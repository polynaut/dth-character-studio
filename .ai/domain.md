...
| **Scene override** | A per-EXTRA-scene delta (`sceneOverrides` on `Character`, schema v17; per-panel gates since v20): four independently-armed panels — **ROM** (`enabled` + `sections`: a per-section snapshot map — the scene's OWN groups for each ROM section it diverges on; a section absent inherits the primary's, so `applySceneOverride` takes `sections[S] ?? base[S].groups`. Schema **v21** replaced the old `poses`/`additions` deltas with this, so a non-primary scene edits its ROM exactly like the primary — free reorder / insert / delete of frames AND groups), **identity** (`identity.enabled` + G9 FACS-detail/flexion/tear-UV), **groom** (`groom.enabled`, a UI gate only — hair lists live per scene in `groomScenes`), **preserve** (`preserve.enabled` + own `morphs`/`nodeTransforms` — a FULL replacement of the base preserve lists, emitted even when empty so a scene can clear them). **Any divergence from the primary IS the override, derived by diffing (never a stored flag):** a row differs in content → the per-row green mark (`sceneRowOverridden`); a section differs in count/order/group-layout → the section-title mark (`sceneSectionDiverged`, a reset back to the primary). The editor writes each snapshot through `pruneSceneSections` (drops a section equal to base, recomputes the ROM `enabled` gate). `applySceneOverride` merges ROM; `mergeSceneOverride` yields the scene's effective character (ROM sections + identity dials). All gates default OFF, so a fresh scene starts fully disabled; arming identity/preserve seeds from the base. `activeSceneOverrides` (any panel armed + scene still in `extraScenes`) is THE single gate; `sceneOverrideBuildsRom` narrows to the ROM subset that also mints a scene-suffixed CSV. A v21 migration folds legacy `poses`/`additions` into `sections` (byte-identical generation). Disabled/unlinked overrides keep their data; their files retire on the next save. |
