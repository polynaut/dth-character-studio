...
| `projects.$projectId.characters.$characterId.tsx` | **The character editor** — draft/save/generate. Decomposed: the route (~640 lines) composes `components/character/*` sections (editor-header — which hosts the DTH Export side panel, `dth-export.tsx` — identity, scripts — which carries the read-only Export directory sub-section — rom-editor, operations-section, rom-run-log-report, scene-footer, frame-zero/preserve/groom fields) + `components/houdini-projects-field.tsx` + `lib/use-scene-selection` / `use-rom-run-log` / `character-paths`. The ROM subtree (`RomSections`/`PoseGroupsEditor`/`GroupCard`) is `React.memo`d with latest-ref, id-routed callbacks — new props into it must keep stable identities or the memo chain silently dies. |
...
**FFI surface: 60 commands** (count re-verified 2026-08-26 — it has drifted
THREE times before, most recently "58" over a real 59; count it, don't trust
it) registered in
...
manifest claims a file for, the Utils drawer's missing-texture product lookup;
