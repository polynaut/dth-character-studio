...
Both lists now carry an **Item** column (schema v32, runtime v74). Picking a
suggestion fills it automatically; it can be edited or cleared by hand. Empty
keeps each list's old reach — every carrier for frame-0 rows, the figure root
for preserve rows — so existing characters generate unchanged. A scoped item
