...
The Daz product scan now recognises **hand-installed content the old matchers could never see** (runtime v92), so far fewer used assets land in "unmatched". The gaps closed — each verified against a real library's scan diagnostics:
...
- A **Content Folder Match** now shows the folder it was identified from in the product's expanded row, so you can see exactly where the content lives.
