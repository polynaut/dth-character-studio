...
- **The export directory is derived, not chosen**: always `<character>/<daz subfolder>/dth-exports`, created with the character and shown read-only on the character page. Existing characters migrate to it on their next save (Tools → Refresh assets does the lot); their old export folders are left on disk untouched.
