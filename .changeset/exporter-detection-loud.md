...
**And it can now tell two very different causes apart.** Daz Studio 6's exporter registers under one class name, Daz Studio 4's under another, so the old class-based lookup reported "not installed" for a plugin sitting right there. It finds both now — but finding it isn't the same as being able to use it. **Scripted export arrived in Exporter Plugin 2.0**, and an older build registers its action normally while offering no scripted export at all, only its own dialog. So a too-old plugin now says exactly that, instead of claiming it isn't installed.
