...
**And it can now tell two very different causes apart.** Daz Studio 6's exporter registers under one class name, Daz Studio 4's under another, so the old class-based lookup reported "not installed" for a plugin sitting right there. It finds both now — but finding it isn't the same as being able to use it: some Exporter builds register the action and still expose no scripted export, only their own dialog. When that happens you're told exactly that, and pointed at the manual dialog, instead of being told the plugin isn't installed when it plainly is.
