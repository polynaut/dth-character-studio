...
**Derived destinations name their true source.** The "Installs into … from the Daz installation above" sentence was hardcoded — even under the Houdini documents folder (derived from the *Houdini* installation) and under Generate Houdini Projects' hython path, which is a tool source, not an install destination. Each now names the section it actually derives from, an empty Houdini documents folder gets the real fix ("start this Houdini once"), and the hython line says "Uses" instead of "Installs into".
