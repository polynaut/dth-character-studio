...
Also: the "Daz scripts generated" path chip matches the Export directory chip's height, and a new `outline-destructive` button (a light-red-bordered destructive style) is used for the export-directory Clear (icon-only ×) and the folder-move Cancel buttons.
